...
const FileState = @import("./repo.zig").FileState;
...
pub const StagedFiles = struct {
    ...
    pub fn load(self: *StagedFiles) !void {
    ...
    pub fn save(self: *StagedFiles) !void {
        const cwd = std.fs.cwd();
        ...
        for (filePaths) |filePath| {
            ...
            while (try walker.next()) |entry| {
                ...
                if (self.files) |*files| {
                    ...
                    const trackedFile = StagedFiles.FileMetadata{
                        .full_path = full_path,
                        .file_name = file_name,
...
// pub fn loadFiles(allocator: std.mem.Allocator, path: []const u8) !std.StringHashMap(FileMetadata) {
//     var staged = StagedFiles.init(allocator, path);
//     defer staged.deinit();
//     try staged.load();
//     return staged.files orelse std.StringHashMap(FileMetadata).init(allocator);
// }
...
test "serialize and deserialize file metadata" {
    ...
    var metadata = StagedFiles.FileMetadata{
    ...
    const deserialized = try StagedFiles.FileMetadata.deserialize(read_file.reader(), allocator);
...
test "load" {
    ...
    var tracked_files = StagedFiles.init(allocator, "test_metadata.bin");
